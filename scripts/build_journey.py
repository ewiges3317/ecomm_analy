...
# Repo-relative paths (portable across machines/OS)
from scripts._paths import ROOT, DATA_PROCESSED

BASE = DATA_PROCESSED / "base"
OUTDIR = DATA_PROCESSED / "analysis"
OUTDIR.mkdir(parents=True, exist_ok=True)

# Load required tables (normalized CSVs written by normalize_types.py)
orders  = pd.read_csv(BASE / "olist_orders.csv", low_memory=False)
items   = pd.read_csv(BASE / "olist_order_items.csv", low_memory=False)
reviews = pd.read_csv(BASE / "olist_reviews.csv", low_memory=False)
cust    = pd.read_csv(BASE / "olist_customers.csv", low_memory=False)
sellers = pd.read_csv(BASE / "olist_sellers.csv", low_memory=False)

# Optional enrich
try:
    products = pd.read_csv(BASE / "olist_products.csv", low_memory=False)
...
journey = (
    orders.merge(agg_items, on="order_id", how="left")
          ...
          .merge(cust[["customer_id","customer_unique_id","customer_city","customer_state"]],
...
journey = journey.merge(sellers[["seller_id","seller_city","seller_state"]],
...
# Write
csv_path = OUTDIR / "journey_delivery_csat_repeat.csv"
par_path = OUTDIR / "journey_delivery_csat_repeat.parquet"
...
print(f"Wrote parquet: {wrote_parquet} -> {par_path.as_posix() if wrote_parquet else '(pyarrow not installed)'}")
