...
# Repo-relative, portable paths
from scripts._paths import RAW_BRAZIL, DOCS

OUT_TXT = DOCS / "join_checks.txt"

# Brazilian e-commerce sources (Olist)
p_orders = RAW_BRAZIL / "olist_orders_dataset.csv"
p_items  = RAW_BRAZIL / "olist_order_items_dataset.csv"
p_pay    = RAW_BRAZIL / "olist_order_payments_dataset.csv"
p_cust   = RAW_BRAZIL / "olist_customers_dataset.csv"
p_prod   = RAW_BRAZIL / "olist_products_dataset.csv"
p_trans  = RAW_BRAZIL / "product_category_name_translation.csv"
...
# items → orders / payments(agg) / customers
oi = items.merge(
    orders[["order_id","customer_id","order_status","order_purchase_timestamp"]],
    on="order_id", how="left"
)
oi_pay = pay.groupby("order_id", as_index=False)["payment_value"].sum().rename(
    columns={"payment_value":"order_payment_sum"}
)
oi = oi.merge(oi_pay, on="order_id", how="left").merge(cust, on="customer_id", how="left")

# product category translation (future dims)
...
# Revenue vs payment sanity
oi["item_total"] = oi["price"] + oi["freight_value"]
rev_vs_pay = (
    oi.groupby("order_id", as_index=False)[["item_total"]].sum()
      .merge(oi_pay, on="order_id", how="left")
)
...
# Counts
lines.append(
    f"[counts] orders={len(orders)} items={len(items)} "
    f"payments={len(pay)} customers={len(cust)} products={len(prod)}"
)
